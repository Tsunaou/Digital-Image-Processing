...
出于计算方便和精度的保留，这里用累积分布直方图来等价代替累积分布函数（区别在于是否除以像素总数目归一化）。设$cdf(v)$为原单通道图的累积分布直方图，$cdf_{min}$为最小的数量非零的灰度值，$M,N$分别为原图的长和宽，$L$为灰度级别（一般为256），则对于灰度$v$,直方图均衡化后得到的结果$h(v)$如下：
...
2. 用```imhist```函数得到该矩阵的直方图，统计每个灰度值有几个像素
...
综上的图像可以看出，和库函数```histeq```相比，我实现的```hist_equal```函数也可以将灰度图像直方图均衡化，并达到类似的效果。其中 gray4 和 gray5 的效果相对而言更加突出一些，尤其是gray5，可以观察到许多原图中不突出的细节，图片的对比度也很好，可以说是一次成功的实验。

观察直方图，可以看到虽然显示效果类似，但是我的函数和库函数的直方图差别还是挺大的，都是在原图的直方图上均衡化，但是库函数灰度分散情况的离散程度更为明显一些，而我的粒度要更细致一些。阅读```histeq```的源码，我猜测是里面有个参数n在起作用，我的n是256，而库函数中为64。因此库函数的均衡化的直方图更加离散。
...
大图按下可打开：[原图](big/r1.jpg)，[RGB](big/r2.jpg)，[HSV](big/r3.jpg)，[HSI](big/r4.jjpg)
...
大图按下可打开：[原图](big/s1.jpg)，[RGB](big/s2.jpg)，[HSV](big/s3.jpg)，[HSI](big/s4.jjpg)

##### (6) 分析与结论

综上的图像可以看出，RGB法分别求均衡化的图片颜色变化比较失真，无法预测。以上面(5)saber.jpg样例的大图为例（样例4、5都是1080p的大图，更有助于观察），原图背景偏暗，而RGB处理后整个人物的颜色变得很淡，效果很不好。而HSV和HSI方法都能挺好地放大对比度，并且加大了亮度，使得图片更加突出一些。纵观5个样例，可以看到HSV和HSI效果都不错，各有优劣。相对而言，HSI对原图的亮度对比提高程度会更好一些。看样例(3) xin.png 可以看到，里面HSI处理后人物的脸变得比较白，但是又没有RGB处理时白得过分。但是相对的，样例(5)中又有一些过曝的感觉，图片右边的黄色显得过亮。感觉HSV法比较稳，但是HSI法比较激进一些吧。

而从直方图来看，RGB法的直方图与灰度图情况一致，不多赘述。而HSV法和HSI法由于是对V和I通道处理，因此此时的直方图并不是单纯地对RGB直方图均衡化，而各有特点。相对而言，HSV均衡化后，直方图会有一种像是两个直方图累加的感觉（一个分散，一个稠密的直方图），而HSI的直方图很有趣地呈现一种类似均匀分布的趋势。